...
Concrete subtypes determine how load/size on an arc is translated into a financial or
performance cost.

Concrete subtypes **must** implement:
- `evaluate(arc_f::T, commodities; presorted=false) -> Float64` — total arc cost for
  `commodities`.
...
- `incremental_cost(arc_f::T, existing, new) -> Float64` — marginal cost of adding `new` to
  an arc that already holds `existing`. Defaults to
  `evaluate(arc_f, existing ∪ new) - evaluate(arc_f, existing)`. Overload it when a closed
  form is cheaper (e.g. `LinearArcCost`, `BinPackingArcCost`).
- `lower_bound_incremental_cost(arc_f::T, existing, new) -> Float64` — a relaxation of
  ...
  (e.g. `BinPackingArcCost`, which relaxes the integer bin count to a fractional one).
"""
