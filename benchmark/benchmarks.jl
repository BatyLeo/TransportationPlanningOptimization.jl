...
const PUBLIC = joinpath(@__DIR__, "..", "test", "public")

function load_instance(name::String)
    nodes_file = joinpath(PUBLIC, "$(name)_nodes.csv")
    legs_file = joinpath(PUBLIC, "$(name)_legs.csv")
    commodities_file = joinpath(PUBLIC, "$(name)_commodities.csv")
